# Implement Journey Updates

## Variables

SPECIFICATION_FILE_PATH: [INSERT SPECIFICATION FILE PATH HERE]
IMPLEMENTATION_FILE_PATH: [INSERT IMPLEMENTATION FILE PATH HERE]

## Description

Take the content from updates to the user journey and integrate them into the implemented code. Follow the build order as specified in the `IMPLEMENTATION_FILE_PATH`.
...
- **Consistency**: Ensure all pages in a journey follow the same styling patterns as existing journeys

### Structural Change Examples

- **New pages**: [Description of the new pages being added]
- **Page order changes**: [Description of the changes to page order]
- **Path changes**: [e.g. `/old-path` → `/new-path`]
- **Deleted pages**: [Description of pages that are no longer needed that will be removed]

# Instructions

Take the instructions and parameters provided, then:

1. **Journey Type**: Focus ONLY on the specified journey
2. **Changes Source**:

- Read and analyze the markdown file content in SPECIFICATION_FILE_PATH
- Compare with the implemented user journey
...
- Focus on ALL content changes including:
  ...
  - **Terminology changes**: Word variations (e.g., "redline" vs "red line")
...
- **Styling Verification**:
  ...
  - Ensure styling consistency with existing journeys
- **Functional Testing**:
