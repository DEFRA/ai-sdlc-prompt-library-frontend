...
SPECIFICATION_FILE_PATH: [INSERT SPECIFICATION FILE HERE]

## Description

Validates and fixes journey specification files to ensure they can be successfully processed by the Coding Agent. This prompt scans the specification file (provided as SPECIFICATION_FILE_PATH above) for common issues, checks for conflicts with existing journey code, and applies fixes to reduce manual tweaks after implementation.
...
- Read the specification file at SPECIFICATION_FILE_PATH
...
Before applying any changes, scan the existing implementation to identify potential conflicts:

### Route Conflicts

- **Scan**: `app/routes/{JOURNEY_NAME}.js` and `app/config/{JOURNEY_NAME}/routes.js`
- **Check for**:
...
- **Scan**: All view files in `app/views/{JOURNEY_NAME}/` for `data.*` references
- **Check for**:
...
- **Scan**: `app/views/{JOURNEY_NAME}/` directory
- **Check for**:
...
- **Scan**: `app/config/{JOURNEY_NAME}/` directory
- **Check for**:
  - Shared configuration files (e.g., display-messages.js)
...
5. Ensure base path matches journey name (e.g., `/payments-journey-2/`)
6. Update the specification with corrected path

**Example transformations:**

```
/payments-journey-2/LPAemail → /payments-journey-2lpa-email
/payments-journey-2/SummaryAndDeclaration → /payments-journey-2/summary-and-declaration
/payments-journey-2/EstimateRef → /payments-journey-2/estimate-ref
/payments-journey-2/do_you_have_ref → /payments-journey-2/do-you-have-ref
```
...
- **Hardcoded URLs only**: Use explicit paths
  ...
  - ✅ Correct: `/payments-journey-2/previous-page`
...
## 14. Update Specification File

After all validations and fixes:

1. **Provide summary of changes**: Create a list of all the modifications made to `SPECIFICATION_FILE_PATH`
2. **Output the fixed specification**: Replace `SPECIFICATION_FILE_PATH` with the modified version. Preserve the original structure and formatting where possible.

## 15. Identify High Risk Features

Review the updated `SPECIFICATION_FILE_PATH`. Identify any significant changes that will be required for this implementation.

Check if any of the following high impact changes will be required:

- New libraries that will be added to the repository
- New calls to 3rd party APIs and other services
- Significant architectural changes to the code
- Complex new features that will be difficult to build
- Complex CSS that is not part of the GOV UK component library

If any of these are found, they must be added to the list of manual review required.

## 16. Make Implementation Plan

Next, make an implementation plan for the updated `SPECIFICATION_FILE_PATH`.
Prioritise end to end functionality before fidelity.
Prioritise high risk and complex features before simple screens.

Output the implementation plan adjacent to `SPECIFICATION_FILE_PATH` with the filename `{feature}.plan.md`.
...
Updated file: SPECIFICATION_FILE_PATH
